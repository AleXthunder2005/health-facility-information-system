...
const addDoctor = async (req, res) => {
  try {
    ...
    // Move the file to the uploads/doctors directory
    ...
    // Save the file
    if (imageFile.buffer) {
      fs.writeFileSync(path.join(uploadDir, imageName), imageFile.buffer);
    } else if (imageFile.path) {
      const fileContent = fs.readFileSync(imageFile.path);
      fs.writeFileSync(path.join(uploadDir, imageName), fileContent);
    } else {
      return res.json({
        success: false,
