...
const VisitResultModal = ({ appointment, onClose, onComplete }) => {
    ...
    const toggleService = (service) => {
        setSelectedServices(prev => prev.find(s => s._id === service._id)
            ...
            <div className="bg-white rounded-2xl p-6 max-w-2xl w-full shadow-xl relative max-h-[90vh] overflow-y-auto">
                ...
                <div className="flex items-center gap-2 mb-3">
                    ...
                    <div>
                        ...
                        <p className="text-gray-500 text-sm">{appointment.slotDate} • {appointment.slotTime}</p>
                    </div>
                </div>

                <textarea placeholder="Жалобы пациента" value={complaints} onChange={e => setComplaints(e.target.value)}
                          className="w-full border p-3 rounded-xl mb-1 text-sm" rows={1} />

                <textarea placeholder="Пометки врача" value={notes} onChange={e => setNotes(e.target.value)}
                          className="w-full border p-3 rounded-xl mb-1 text-sm" rows={1} />

                <textarea placeholder="Рекомендации" value={recommendations} onChange={e => setRecommendations(e.target.value)}
                          className="w-full border p-3 rounded-xl mb-1 text-sm" rows={1} />
                ...
                <div className="flex flex-col gap-1 max-h-32 overflow-y-auto mb-4">
                    {allServices.map(s => (
                        <div key={s._id} onClick={() => toggleService(s)}
                             className={`flex justify-between border px-3 py-1 rounded-lg cursor-pointer
                                ${selectedServices.find(sel => sel._id === s._id) ? "bg-primary text-white" : "hover:bg-gray-50"}`}>
                            <span>{s.name}</span>
                ...
                <button onClick={submitVisit} className="w-full bg-primary text-white py-1 rounded-xl hover:bg-primary-light transition">
                    Завершить приём
                </button>
            </div>
        </div>
